...
Publishes newsletter posts to beehiiv using browser automation when the
v2 REST API is unavailable (e.g., free Launch plan). Logs in with
BEEHIIV_EMAIL and BEEHIIV_PASSWORD — no paid API tier required.
...
_LOGIN_URL = "https://app.beehiiv.com/login"
...
def publish_via_browser(
    ...
    """
    email = os.getenv("BEEHIIV_EMAIL", "")
    password = os.getenv("BEEHIIV_PASSWORD", "")

    if not email or not password:
        return {
            "success": False,
            "post_url": None,
            "error": "BEEHIIV_EMAIL or BEEHIIV_PASSWORD environment variable not set",
        }
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            msg = f"Timed out waiting for beehiiv UI element. Check credentials and publication_id. Details: {e}"
            logger.error(msg)
            result["error"] = msg
...
def _login(page, email: str, password: str) -> None:
    """Navigate to beehiiv login and authenticate."""
    logger.info("Navigating to beehiiv login...")
    page.goto(_LOGIN_URL, wait_until="domcontentloaded", timeout=_TIMEOUT)

    # Fill credentials — beehiiv uses standard email/password fields
    page.get_by_label("Email").fill(email)
    page.get_by_label("Password").fill(password)
    page.get_by_role("button", name=re.compile(r"sign\s*in|log\s*in", re.I)).click()

    # Wait for redirect away from login page — means auth succeeded
    page.wait_for_url(re.compile(r"app\.beehiiv\.com/(?!login)"), timeout=_TIMEOUT)
    logger.info("beehiiv login successful")
...
def _insert_html_block(page, html_content: str) -> None:
    ...
    If beehiiv's UI changes the slash-command, the fallback tries the "+" button.
    """
    ...
    if not html_picked:
        # Try any visible item that contains "HTML" text
...
def _fill_html_editor(page, html_content: str) -> None:
    ...
    2. CodeMirror 6 / generic contenteditable editor (dispatch transaction)
    3. Plain <textarea> via fill()
    Uses page.evaluate() to avoid the slowness of keyboard.type() for large HTML.
    """
    # Attempt to set content via JS — handles CodeMirror and plain textarea
    ...
    if filled:
        ...
        # Final fallback: use clipboard paste
        logger.warning("JS editor injection failed; attempting clipboard paste")
        page.evaluate(
            "(html) => navigator.clipboard.writeText(html)", html_content
        )
        page.keyboard.press("Control+a")
...
def _schedule_post(page, scheduled_at: str) -> None:
    ...
    beehiiv's scheduling UI typically shows date + time pickers.
    """
    ...
    # beehiiv uses separate inputs for month, day, year, hour
    ...
    _fill_if_visible(
        ['input[placeholder="DD"]', 'input[name="day"]'],
        str(dt.day).zfill(2),
    )
    _fill_if_visible(
    ...
    _fill_if_visible(
        ['input[placeholder="HH"]', 'input[name="hour"]'],
        str(dt.hour).zfill(2),
    )
    _fill_if_visible(
        ['input[placeholder="MM"]', 'input[name="minute"]'],
        "00",
    )
